...
import { writeGroupsSnapshot, writeTasksSnapshot } from '../ipc/snapshots.js';
import { logger } from '../logger.js';
import { AvailableGroup, Channel, MessageAttachment, RegisteredGroup } from '../types.js';

import { findChannel, formatMessages, stripInternalTags } from './router.js';
import { hasTriggerMessage } from './trigger.js';
...
export async function processGroupMessages(chatJid: string, state: AppState, deps: ProcessDeps): Promise<boolean> {
  ...
  if (!isMainGroup && group.requiresTrigger !== false && !hasTriggerMessage(missedMessages)) return true;
