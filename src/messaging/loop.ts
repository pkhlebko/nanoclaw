...
import { ASSISTANT_NAME, MAIN_GROUP_FOLDER, MODEL_FLAG_REGEX, POLL_INTERVAL } from '../config.js';
import { getMessagesSince, getNewMessages } from '../db/messages.js';
...
function routeGroupMessages(chatJid: string, groupMessages: NewMessage[], state: AppState, deps: LoopDeps): void {
  ...
  if (needsTrigger && !hasTriggerMessage(groupMessages)) return;
